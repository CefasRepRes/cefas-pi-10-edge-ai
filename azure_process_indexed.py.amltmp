...
def movedatasettoazurecomputeinstance(wsname, wssubscription_id, wsresource_group, dsname, datastorename):
    ...
    datastore_paths = [(datastore, dsname)] # need to use 
    print(datastorename)
    print(dsname)
...
def parallel_size_images(file_paths):
    with Pool() as pool:
        size_info = pool.map(partial(extract, return_contours=False), file_paths)
    return size_info
...
if __name__ == "__main__":
    ...
    if args.model_version == 2:
        versionname = "./models/model_18_21May.pth"

    if args.model_version == 0:
        versionname = "./models/model_18_pre.pth"

    model = load_model(versionname,
        device,
        "resnet18"
    )
    ...
    image_batch_size = 250000 # How many you should load in parallel depends on the RAM of your compute instance
...
#    all_scores = []
    ...
    for batch_images, batch_paths in load_images_in_batches(filenames_list, image_batch_size):
        ...
        sz_df = pd.DataFrame(sz_info, columns=["esd_1", "esd_2", "threshold_area", "threshold_area_james_osho", "object_length", "max_points"], index=batch_paths)
        ...
        combined_df['object_length'] = combined_df['object_length'].round() # rounding to the nearest pixel is a reasonable way to compress the output, we cannot justify greater precision really
        combined_df['Latitude'] = combined_df['Latitude'].round(4)        
        combined_df['Longitude'] = combined_df['Longitude'].round(4)
        combined_df['Image DateTime'] = combined_df['Image DateTime'].str.replace(':', '-', 2)
        combined_df['Image DateTime'] = pd.to_datetime(combined_df['Image DateTime']).dt.round('min')
        aggregated_combined_df = combined_df.groupby(['Latitude', 'Longitude', 'Image DateTime', 'Predicted Class', 'object_length'],as_index=False).size()
        aggregated_combined_df = aggregated_combined_df.rename(columns={'size': 'count'})
        

        # Write combined DataFrame to CSV
        fname=f"{args.dsname}_batch_{batchi}_combined_model_v_{versionname}.csv".replace('/', '').replace('\\', '')
        if not os.path.exists("./temporary/"):
            os.makedirs("./temporary/")
        output_file = f"./temporary/{fname}"
        aggregated_combined_df.to_csv(output_file, index=False)


        # Upload to Azure Blob Storage
        datastore = get_datastore_path(ws, args.datastorename)
        blob_file_path = f"classifications_to_nearest_pixel/{os.path.basename(output_file)}"
