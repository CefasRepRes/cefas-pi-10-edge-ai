...
DEFAULT_LABELS = ["Copepod", "Detritus", "Noncopepod"]
LABELS = shorten_and_unique_labels(DEFAULT_LABELS)
...
def set_labels(labels):
    """Set classifier labels at runtime from training_report*.json classes."""
    global LABELS
    if not labels:
        raise ValueError("Training report does not contain any classes")
    LABELS = shorten_and_unique_labels(list(labels))
    return LABELS


def load_training_report(path):
    """Load a training_report*.json file."""
    with open(path, "r", encoding="utf-8") as f:
        return json.load(f)


def _temperature_from_report(training_report):
    """Read optional calibration temperature from a training report."""
    if not training_report:
        return 1.0
    calibration = training_report.get("calibration", {}) or {}
    try:
...
def load_model(filename, device, model_version=None, labels=None, training_report=None):
    """Load model weights using runtime architecture/classes when supplied."""
    if training_report:
        labels = labels or training_report.get("classes")
        model_version = model_version or training_report.get("arch")
    ...
    elif model_version == "resnet18_gray":
        model = resnet18_gray(num_classes=len(LABELS))
    else:
        raise ValueError(f"Unsupported model architecture: {model_version}")
    ...
    model.inference_temperature = _temperature_from_report(training_report)
...
def classify(image, device, model, gray=False):
    ...
    with torch.set_grad_enabled(False):
        outputs = model(t)
        temperature = getattr(model, "inference_temperature", 1.0)
        scores = torch.softmax(outputs / temperature, dim=1)  # Calculate calibrated softmax scores
...
def classify_batch(image_list, device, model, gray, batch_size=1):
    ...
    with torch.set_grad_enabled(False):
        #print("Batch shape:", t_batch.shape)
